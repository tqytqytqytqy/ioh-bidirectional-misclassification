...
def _write_nibp_subset_representativeness(cfg: dict, mechanism_case: pd.DataFrame, events: pd.DataFrame, panel: pd.DataFrame) -> None:
    ...
    failed_cases = set(pairs.loc[~pairs["paired"], "case_id"].drop_duplicates()) if not pairs.empty else set()
    ...
    groups = {
        ...
        "failed_pairing_cases": failed_cases,
    }
    variables = [
        ("age", "mean"),
        ("sex_male", "mean"),
    ...
    for group, cases in {"nibp_mechanism_subset": mechanism_cases, "failed_pairing_cases": failed_cases}.items():
        sub = m[m["case_id"].isin(cases)]
    ...
    qc_path(cfg, "nibp_subset_smd.csv").write_text(pd.DataFrame(smd_rows).to_csv(index=False), encoding="utf-8")
            ...
            [
                {
                    ...
                    "role": "VitalDB-to-target-population transportability sensitivity; not a validation claim",
...
def make_manuscript_numbers(cfg: dict) -> dict:
    ...
    report = [
        "# IOH2 BJA Current Results Summary",
        "",
        "## Main boundary",
        "",
...
def make_manuscript_inputs(cfg: dict) -> None:
    ...
    main_md = manuscript_dir / "main_bja_revised.md"
...
"""
    ...
    doc.save(manuscript_dir / "main_bja_revised.docx")
    ...
    (manuscript_dir / "supplement_bja_revised.md").write_text(supplement_text, encoding="utf-8")
    supp = Document()
    ...
    supp.save(manuscript_dir / "supplement_bja_revised.docx")
    ...
    for name in ["STROBE_RECORD_checklist.docx", "diagnostic_reporting_checklist.docx"]:
        ...
        checklist.add_paragraph("Checklist generated for the current BJA bidirectional exposure-misclassification revision. Final author sign-off required before journal submission.")
